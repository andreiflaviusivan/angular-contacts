...
import { selectIsUserPresentState, selectIsInAnyOfRolesState, selectUpdateUserStateBeat } from '../reducers';
import { tap, exhaustMap, last, delay, first, filter, map } from 'rxjs/operators';
                    ...
                    exhaustMap(auth => {
                        if (auth) {
                            if (roles != null && roles.length > 0) {
                                return this.store.select(selectIsInAnyOfRolesState(roles)).pipe(
